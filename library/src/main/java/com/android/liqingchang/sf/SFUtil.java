...
public class SFUtil {
    ...
    public static String getVerifyCode(String xml) {
        String data = xml + SFConstants.CHECKWORD;
//        String data = "<Requestservice=OrderService>" + SFConstants.CHECKWORD;
        Log.i("terry", data);
        byte[] md5 = md52(data);
        String ret = Base64.encodeToString(md5, Base64.DEFAULT);
        Log.i("terry", ret);
        return ret;
    }
...
//			/* 从文件读取数据到缓冲区 */
     ...
     * 获取文件对象
     */
    public static File getFile(Context context, String documentPath, String fileName) {
        File path = new File(context.getFilesDir() + documentPath);
        if (!path.exists()) {
            path.mkdirs();
        }
